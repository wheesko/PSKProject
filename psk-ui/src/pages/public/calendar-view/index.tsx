...
import {Calendar, Badge, Modal, Form, Input, Button, TimePicker} from 'antd';
import * as moment from 'moment';
import {StatusType} from '../../../models/status-type-model';

import './CalendarStyles.css';
import {DAY_EVENT_LIST_EMPTY} from '../../../Constants';
...
const CalendarView: React.FunctionComponent<{}> = () => {
    ...
    const [modalDate, setModalDate] = useState<moment.Moment | undefined>(undefined);
    ...
    const handleSelect = (date?: moment.Moment | undefined): void => {
        ...
        if (date !== undefined)
            setModalListData(getListData(date));
    };
    ...
    return (
        <div>
            ...
            />
            <Modal
                title={modalDate === undefined ? null : `${modalDate.get('year')}-${modalDate.get('month')}-${modalDate.get('date')}`}
                visible={modalVisibility}
                onOk={handleOnOk}
                onCancel={handleOnCancel}
            >
                {modalListData.length === 0 ? null : modalListData.map(item => (
                    <li key={item.content}>
                        <Badge status={item.type} text={item.content}/>
                    </li>
                ))}
                {modalDate === undefined ? null : getListData(modalDate).length === 0 ?
                    <p>{DAY_EVENT_LIST_EMPTY}</p> : null}
